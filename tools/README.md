...
These conversions were validated in [`tractorjuice/arc-kit`](https://github.com/tractorjuice/arc-kit/tree/main/tests/mermaid-wardley):

- **`test-real-maps.mjs`** — parses every converted map with the real Mermaid parser (requires `mermaid` + `jsdom` + `dompurify`). Result: 144 / 147 maps parse; the 3 failures are source-data errors in the originals (typos, malformed coordinates).
